...
sudo -i
curl -k -XPOST https://vhd.linnovate.net/service?sw=Azure-ARM-Rancher3

info_file=~/information_on_$0.txt
...
do
    sleep 5
    echo "starting with installation"| tee -a $info_file
done
...
echo "installing kubectl" | tee -a $info_file
...
echo "adding pub key" | tee -a $info_file
...
cd /usr/local/bin
sudo wget https://github.com/rancher/rke/releases/download/v1.3.1/rke_linux-amd64
...
sudo bash -c 'echo "Your rancher should be is available as https://'$fqdn'">> /etc/motd'
sudo bash -c 'echo "initial password should be '$pass'">> /etc/motd'
